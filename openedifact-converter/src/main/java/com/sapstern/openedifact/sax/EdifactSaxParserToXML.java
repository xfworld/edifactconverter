...
public class EdifactSaxParserToXML extends AbstractEdifactParser implements XMLReader, EdifactSaxParserToXMLIF {
	 ...
	 * @throws SAXException
	 */
	public static EdifactSaxParserToXMLIF factory(String encoding, java.util.logging.Logger logger)
			throws SAXException {
	 ...
	 * @throws SAXException
	 */
	public static EdifactSaxParserToXMLIF factory(String encoding, java.util.logging.Logger logger, boolean isNamespace)
			throws SAXException {
		EdifactSaxParserToXML theParser = new EdifactSaxParserToXML(encoding, logger);
	...
	private EdifactSaxParserToXML(String encoding, java.util.logging.Logger logger, boolean isNamespace)
			throws SAXException {
		super();
		this.logger = logger;
	 ...
	 * 
	 * @param EdifactSegment currentSegment: The current EdifactSegment to be
	 *                       processed by our reverse SAX parser
	 * @throws SAXException
	 */
	private void parseCurrentSegment(EdifactSegment currentSegment) throws SAXException {
		...
		for (int count = 0; count < eFields.size(); count++) {
			...
			if (fieldObject.isComposite) {
				contentHandler.startElement(namespaceURI, nameSpace, fieldObject.fieldTagName, null);
				...
				contentHandler.startElement(namespaceURI, nameSpace, fieldObject.fieldTagName, attribs);
				contentHandler.characters(value.toCharArray(), 0, value.length());
			}
			contentHandler.endElement(namespaceURI, nameSpace, fieldObject.fieldTagName);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.sapstern.openedifact.sax.EdifactSaxParserToXMLIF#parse(org.xml.sax.
	 * InputSource)
	 */
	@Override
	public void parse(InputSource source) throws IOException, SAXException {
		...
		java.io.BufferedInputStream bis = new java.io.BufferedInputStream(source.getByteStream());
		...
		// Hint of orca007 iso-2022 .*UNB\\+UNOX.*
		if (rawEdifact.matches("(?s).*UNB\\+UNOC.*|.*UNB\\+UNOX.*|.*UNB\\+UNOY.*")) {
			// we are fine do nothing
			logger.log(Level.FINE, "match .*UNB\\+UNOC.*");
		} else if (rawEdifact.matches("(?s).*UNB\\+UNOB.*|.*UNB\\+UNOA.*")) {
		...
		if (isNamepace) {
			...
			nameSpace = "ns0";
			rootAttribs.addAttribute("http://www.w3.org/2001/XMLSchema-instance", nameSpace, "xmlns:" + nameSpace,
					"CDATA", namespaceURI);
		} else
			rootAttribs.addAttribute("http://www.w3.org/2001/XMLSchema-instance", nameSpace, "xmlns:ns0", "CDATA",
					namespaceURI);
		rootAttribs.addAttribute("http://www.w3.org/2001/XMLSchema-instance", "xmlns:xsi", "xmlns:xsi", "CDATA",
				"http://www.w3.org/2001/XMLSchema-instance");
		rootAttribs.addAttribute("http://www.w3.org/2001/XMLSchema-instance", "", "xsi:noNamespaceSchemaLocation",
				"CDATA", xsdFileName + ".xsd");
	 ...
	 * 
	 * @see
	 * com.sapstern.openedifact.sax.EdifactSaxParserToXMLIF#getProperty(java.lang.
	 * String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.sapstern.openedifact.sax.EdifactSaxParserToXMLIF#setProperty(java.lang.
	 ...
	 */
	private EdifactSegment setupTreeOfEdifactSegments(String flatEdifact, String messageName)
		...
		logger.finest(new java.util.Date(System.currentTimeMillis()).toString() + ": "
	 ...
	 * 
	 * @param flatEdifactMessage : the input message string
	 * @param segmentDelimiter   : most cases '
	 * @param subFieldDelimiter  : most cases :
	 * @return : the message name / the message version / the UNA segment content
	 * @throws SAXException : thrown if we can't find message name and / or version
	 */
	private Hashtable<String, String> getInitialValues(String flatEdifactMessage, String segmentDelimiter,
			String subFieldDelimiter) throws SAXException {
	...
	private Hashtable<String, String> getMessageNameAndVersion(String subFieldDelimiter, String theUnhSegment) {
		...
		StringTokenizer theFieldTokenizer = new StringTokenizer(theUnhSegment, fieldDelimiter);
		while (theFieldTokenizer.hasMoreElements())
			theParseString = theFieldTokenizer.nextToken();
		StringTokenizer theTokenizer = new StringTokenizer(theParseString, subFieldDelimiter);
		...
		if (currentDomElement.hasChildNodes()) {
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					// Hier kommt ggf eine Liste zurueck, weil wir mehrere Edifact Segmente des
					// gleichen Typs hintereinander haben koennen
	...
	protected List<EdifactSegment> findEdifactSegmentForElement(Element currentElement) {
		...
		for (int j = 0; j < theCildNodesOfComplexType.size(); j++) {
			...
			if (nameOfAttrib.startsWith("G_") | nameOfAttrib.startsWith("M_")) {
				...
				// Element theCheckElement = getElementByAttributeValueFromDOM( nameOfAttrib,
				// this.theDOMElementList );
				Element theCheckElement = (Element) this.theElementTab.get(nameOfAttrib);
				List<Element> theCheckCildNodesOfComplexType = getNodeListOfComplexType(
						theCheckElement.getChildNodes());
				for (int i = 0; i < maxOccurs; i++) {
					...
					// Element theElement = getElementByAttributeValueFromDOM(nameOfAttrib,
					// this.theDOMElementList);
			...
			while (!rawListOfAllEdifactSegments.isEmpty()) {
				...
				if (currentSegment.segmentName.equals(nameOfAttrib)) {
					...
					// Assign associated DOM Element to segment object we need it later while
					// setting up the fields of the segment
					// currentSegment.theElement = getElementByAttributeValueFromDOM(nameOfAttrib,
					// this.theDOMElementList);
					...
					// lookup of next segment,if it has a different segment name we get out of the
					// loop
	 ...
	 * objects<br>
	 * 
	 * @param EdifactSegment segmentObject: The root EdifactSegment object
	 *                       containing<br>
	 *                       all the other children EdifactSegment segment
	 *                       objects<br>
	...
	protected EdifactSegment refineEdifactSegment(EdifactSegment segmentObject) throws SAXException {
		...
		// Hier muss ueber den aktuellen DOM Subtree geloopt werden und nicht ueber den
		// Edifact String
		...
		for (int i = 0; i < childElementList.size(); i++) {
			...
			if (nameOfAttrib.startsWith("C_")) {
				...
				if (currentToken.equals(fieldDelimiter)) {
					...
					if (tokenList.size() <= index)
						break;
					// This should always be as the tokenizer produces the delimiters as tokens,
					...
					if (currentToken.equals(fieldDelimiter))
						...
						nextNextIndex++;
						if (tokenList.size() > nextIndex && tokenList.size() >= nextNextIndex
								&& tokenList.get(nextIndex).equals(fieldDelimiter)
								&& tokenList.get(nextNextIndex).equals(fieldDelimiter)) {
				...
				for (int j = 0; j < childElementsOfChildList.size(); j++) {
					...
					if (currentSubFieldToken.equals(componentDataSeparator)) {
						...
						EdifactSubField currentSubField = new EdifactSubField(theCurrentElement.getAttribute("ref"),
								currentSubFieldToken, releaseChar);
	 ...
	 * @return
	 */
	private int processDOMElementToken(List<String> tokenList, int index, String nameOfAttrib,
			List<EdifactField> fieldList) {
		// get token
		String currentToken = tokenList.get(index);

		if (!currentToken.equals(fieldDelimiter)) {
			// found a value for the current field from DOM so we setup a field and attach
			// it to list of fields
	 ...
	 * 
	 * @see
	 * com.sapstern.openedifact.sax.EdifactSaxParserToXMLIF#setupRawSegmentList(java
	 * .lang.String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.sapstern.openedifact.sax.EdifactSaxParserToXMLIF#addSubField(java.lang.
	 * String, java.lang.String,
	 * com.sapstern.openedifact.sax.parse.segments.EdifactField)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.sapstern.openedifact.sax.EdifactSaxParserToXMLIF#parse(java.lang.String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.sapstern.openedifact.sax.EdifactSaxParserToXMLIF#parseEdifact(java.lang.
	 * String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.sapstern.openedifact.sax.EdifactSaxParserToXMLIF#parseEdifact(org.xml.sax
	 * .InputSource)
	 */
	@Override
	public ByteArrayOutputStream parseEdifact(InputSource inputEDIFACT)
	...
	/**
	 * Input wasawasa 20191111 Trennzeichen am Ende sind im Edifact zulässig, aber
	 * nicht sinnvoll. Die bringen auch den Parser ins stolpern. Bevor ich den
	 * Parser umbaue, werfe ich den unnötigen Ballast einfach weg.
	 */
	protected void stripSegmentString(EdifactSegment segmentObject) {
		...
		while (true) {
			if (segmentString.length() < 2)
				return;
			// Ist das vorletzte Zeichen der Releas Character, dann darf am Ende ein
			// Delimiter stehen
			...
			if (sc == releaseChar.charAt(0))
				return;
			// Wenn das letzte Zeichen kein Separator und kein Delimiter ist, ist auch gut.
			if (!(lc == fieldDelimiter.charAt(0)) && !(lc == componentDataSeparator.charAt(0)))
				return;
